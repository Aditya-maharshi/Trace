...
import { logApiTrace, requestContextStorage } from "./logger";
...
export class EtherscanProvider implements BlockchainDataProvider {
  ...
  constructor(baseUrl?: string, apiKey?: string) {
    this.baseUrl = baseUrl ?? process.env.ETHERSCAN_BASE_URL ?? "https://api.etherscan.io/api";
    this.apiKey = apiKey ?? process.env.ETHERSCAN_API_KEY ?? "";
  }

  async getTransactions(address: string, pageSize = 100): Promise<Transaction[]> {
    return fetchAccountEndpoint<Transaction>(
      this.baseUrl,
      this.apiKey,
      this.limiter,
      this.name,
  ...
  async getTokenTransactions(address: string, pageSize = 100): Promise<TokenTransaction[]> {
    return fetchAccountEndpoint<TokenTransaction>(
      this.baseUrl,
      this.apiKey,
      this.limiter,
      this.name,
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(data.items)) {
            return data.items.slice(0, pageSize).map((it: any) => ({
              hash: String(it.hash || ""),
...
async function executeWithFallback<T>(
  ...
  try {
    ...
    console.warn(
      `[dataProvider] Fallback provider (${fallbackProvider.name}) succeeded for ${address} (${fallbackResult.length} items).`,
    );
    await writeCache(cacheKey, fallbackResult);
    return {
      data: fallbackResult,
      provenance: {
