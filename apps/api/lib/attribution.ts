...
import type { PathResult } from "./graphBuilder";
import type { Transaction, TokenTransaction } from "./etherscan";
...
export function scorePathDetailed(
  path: PathResult,
  txs: (Transaction | TokenTransaction)[],
): ScoreBreakdown {
  // Guard: if there are no transactions, we can't score meaningfully.
  if (txs.length === 0) {
    return {
      ...
      daysSinceLastTx: 0,
      recencyFactor: 1,
      hopFactor: 1 / Math.max(1, path.depth),
      valueFactor: 0,
      score: 0,
    };
  }
  ...
  const hopFactor = 1 / hops;

  // ── Total value in USD ──────────────────────────────────────────────────
  const totalValueUSD = txs.reduce((sum, tx) => sum + txValueToUSD(tx), 0);
  const valueFactor = Math.log(1 + totalValueUSD);

  // ── Recency ─────────────────────────────────────────────────────────────
  let mostRecentTimestamp = "0";
  for (const tx of txs) {
    if (parseInt(tx.timeStamp, 10) > parseInt(mostRecentTimestamp, 10)) {
      mostRecentTimestamp = tx.timeStamp;
    }
  }
  const daysSinceLastTx = daysSince(mostRecentTimestamp);
  const recencyFactor = Math.exp(-0.01 * daysSinceLastTx);

  // ── Final score ─────────────────────────────────────────────────────────
  const score = hopFactor * valueFactor * recencyFactor;

  return { hops, totalValueUSD, daysSinceLastTx, recencyFactor, hopFactor, valueFactor, score };
}
...
export function scorePath(
  ...
  return scorePathDetailed(path, txs).score;
}
...
export const CALIBRATION_METADATA = {
  ...
  calibratedLowThreshold: 0.00,
  activeHighThreshold: 8,
  activeMediumThreshold: 3,
...
export async function aggregateAttributions(
  ...
  for (const p of paths) {
    // Fetch transactions for the last hop (the link into the VASP)
    // This gives us value and recency data for scoring.
    const precedingAddress = p.path.length >= 2 ? p.path[p.path.length - 2] : p.path[0];
    const txs = await fetchTxs(precedingAddress);

    // Use scorePathDetailed to get both the score and the breakdown terms
    const breakdown = scorePathDetailed(p, txs);
